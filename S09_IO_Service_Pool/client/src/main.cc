...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < 1; ++i) {
    threads.emplace_back([]() -> int {
      try {
        ...
        auto recv_head_node =
            std::make_shared<MsgNode>(MSG_ID_LEN + MSG_DATA_LEN);
        ...
        while (j < 500) {
          ...
          memcpy(&recv_msg_len, recv_head_node->data + MSG_ID_LEN,
                 MSG_DATA_LEN);
  ...
  std::cout << "Cost time:" << std::chrono::duration_cast<std::chrono::milliseconds>(end_time - start_time).count();
  return 0;
}
