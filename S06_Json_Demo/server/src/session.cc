...
void CSession::start(){
	...
	_socket.async_read_some(asio::buffer(_data, MAX_LENGTH), std::bind(&CSession::read_callback_, this, 
...
void CSession::send(std::string msg) {
	...
	asio::async_write(_socket, asio::buffer(msgnode->_data, msgnode->_total_len),
		std::bind(&CSession::write_callback_, this, std::placeholders::_1, shared_self()));
}
...
void CSession::write_callback_(const asio::error_code& error, std::shared_ptr<CSession> shared_self) {
	//增加异常处理
	try {
		if (!error) {
			std::lock_guard<std::mutex> lock(_send_lock);
...
void CSession::read_callback_(const asio::error_code& error, size_t  bytes_transferred, std::shared_ptr<CSession> shared_self){
	try {
		if (!error) {
			//已经移动的字符数
			int copy_len = 0;
				...
				if (!_b_head_parse) {
					...
					if (bytes_transferred + _recv_head_node->_cur_len < HEAD_LENGTH) {
						...
						_socket.async_read_some(asio::buffer(_data, MAX_LENGTH),
							std::bind(&CSession::read_callback_, this, std::placeholders::_1, std::placeholders::_2, shared_self));
						return;
					}
					//收到的数据比头部多
					//头部剩余未复制的长度
